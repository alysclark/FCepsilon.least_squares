...
simulation = oc.openSimulation('FCepsilonRI_Faeder_model2_PI3K.cellml')
...
constant_parameter_names.remove('FCepsilonRI/k_f7')
constant_parameter_names.remove('FCepsilonRI/k_r7')
...
parameter_bounds[1][state_parameter_names.index('FCepsilonRI/Grb2')] = 1000
parameter_bounds[0][state_parameter_names.index('FCepsilonRI/PI3K')] = 0
parameter_bounds[1][state_parameter_names.index('FCepsilonRI/PI3K')] = 1000
parameter_bounds[0][constant_parameter_names.index('FCepsilonRI/k_f8')] = 0.00023
parameter_bounds[1][constant_parameter_names.index('FCepsilonRI/k_f8')] = 70

parameter_bounds = tuple(parameter_bounds)

initial_params=np.zeros((11,1))
...
def model_function_lsq(params, expt_time, expt_pFC, expt_pSyk, return_type, debug=False):
    ...
    if return_type == 'optimisation':
        pSyk = (results.states()[expt_state_uri_pSyk].values()[expt_time])*2
        pFC = (results.states()[expt_state_uri_pFC].values()[expt_time])*1
        f1 = (pSyk-expt_pSyk)
        f2 = (pFC-expt_pFC)
        #print (f1, f2)
